...
function [uNew, d, iter] = jacobi(uNew, uOld,uSwap, f, N, iter_max, gridSpace, tolerance) 
    invCube = 1/6;
    d=10e+5;
    iter = 0;
        ...
        for i = 2:N-1
            for j = 2:N-1
                for k = 2:N-1
                    ...
                    uNew(i,j,k) = invCube*(uOld(i-1,j,k) + uOld(i+1,j,k) + uOld(i,j-1,k) + uOld(i,j+1,k) + uOld(i,j,k-1) + uOld(i,j,k+1) + gridSpace*f(i,j,k));

    ...
    for (i = 1; i < N-1; i++) {
      for (j = 1; j < N-1; j++) {
             for (k = 1; k < N-1; k++) {
                ...
                d_u[i*N*N+j*N+k] = frac*(d_uOld[(i-1)*N*N+j*N+k] + d_uOld[(i+1)*N*N+j*N+k] + 
                d_uOld[i*N*N+(j-1)*N+k] + d_uOld[i*N*N+(j+1)*N+k] + d_uOld[i*N*N+j*N+(k-1)] + d_uOld[i*N*N+j*N+(j+1)] + delta2*d_f[i*N*N+j*N+k]);
            //d += abs(uNew[i*N*N+j*N+k] - uOld[i*N*N+j*N+k]);
            }
        }
    }
}
